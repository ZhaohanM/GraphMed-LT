...
def log_info(message, logger_name="message_logger", print_to_std=False, mode="info"):
    ...
    if logger: 
        ...
        elif mode == "warning": logger.warning(message)
...
class ModelCache:
    def __init__(self, model_name, use_vllm=False, use_api=None, **kwargs):
        ...
        self.client = None
        self.args = kwargs
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # ✅ 添加 device
    ...
    def load_model_and_tokenizer(self):
        if self.use_api == "openai":
            ...
            try:
                ...
                log_info(f"[ERROR] [{self.model_name}]: VLLM fallback to HuggingFace due to error: {str(e)}", mode="error")
                self.use_vllm = False
    ...
    def generate(self, messages):
        ...
        if self.use_api == "openai": return self.openai_generate(messages)
